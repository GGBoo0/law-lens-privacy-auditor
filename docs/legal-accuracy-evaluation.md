...
현재 1단계 runner는 문서·법령 manifest 해시 고정, canonical rule별 점추정치, 두 평가 모드 분리, 독립 라벨과 조정본 연결, 표본 무결성 및 공개 `calibration_pending` 계약 검사를 구현합니다. 필드별 합의도, Wilson 구간과 회사 단위 bootstrap은 실제 calibration 분포를 확인한 뒤 구현·검증해야 하는 **인증 전 필수 작업**입니다. 이 작업과 전문가 표본 요건이 끝날 때까지 `certified`나 성능 게이트를 활성화하지 않습니다.
...
현재 runner는 조정 전 축약 decision의 원시 일치율과 Cohen's κ를 계산합니다. 인증 전에는 원래의 구조화 라벨을 유지한 채 다음 필드별 지표를 추가로 구현합니다.

- 적용 여부·gold label: 원시 일치율과 Cohen's κ
- severity: 선형 가중 Cohen's κ
- evidence support: 원시 일치율과 Cohen's κ
- span 위치: 최적 매칭 token overlap F1을 진단값으로 표시
...
인증 단계에서는 비율 지표에 95% Wilson 구간을, F1에는 고정 seed의 회사 단위 clustered bootstrap 95% 구간을 사용합니다. 현재 1단계 runner는 점추정치만 출력하므로 이 구간 계산을 구현·회귀 검증하기 전에는 성능 게이트를 활성화하지 않습니다. 같은 회사의 여러 규칙 행을 서로 독립이라고 가정하지 않습니다.
...
| 지표 | 잠정 기준 |
| --- | ---: |
| Rule-balanced actionable macro F1 | 0.85 이상 |
| High-risk recall strict | 0.90 이상 |
| Strict evidence grounding | 0.95 이상 |
| Possible-missing precision | 0.90 이상 |
| Unsafe high escalation | 0 |
| Decision Cohen's κ | 0.80 이상 |
...
7. 필드별 합의도·Wilson 구간·회사 단위 bootstrap 구현 검증

현재 설정은 신뢰구간과 필드별 합의도를 `required_before_certification`으로 표시하며, evaluator도 이 상태에서는 `enforced` 인증을 실패시킵니다. 구현·검증 PR에서 두 capability 상태를 `implemented`로 바꾸기 전에는 점추정치만으로 인증할 수 없습니다.
