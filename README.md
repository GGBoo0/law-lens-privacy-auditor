...
법령별 공포번호·공포일·시행일·현행/시행예정 상태와 조문 해시를 [`data/legal-source-snapshot.json`](./data/legal-source-snapshot.json)에 저장합니다. 변경이 없으면 파일이나 배포를 수정하지 않습니다. 변경이 확인되고 열린 자동화 PR이 없으면 실행별 고유 브랜치와 Draft PR을 생성합니다. 열린 자동화 PR이 있으면 그 브랜치의 최신 스냅샷을 기준으로 다시 확인하고, 새 변경이 있을 때 생성 파일 2개만 새 봇 커밋으로 일반 푸시합니다.
