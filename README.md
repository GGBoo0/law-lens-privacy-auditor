...
GitHub Actions의 [`Legal source monitor`](./.github/workflows/legal-update-monitor.yml)가 매일 오전 9시 17분(Asia/Seoul)에 다음 공식 소스 9개를 확인합니다.

- 법제처 국가법령정보 공동활용 API의 현행·시행예정 개인정보 보호법과 시행령
- 전자상거래법 시행령·인공지능기본법·위치정보법·신용정보법
- 개인정보의 안전성 확보조치 기준
- 개인정보보호위원회 현재 안내서 목록과 개인정보 처리방침 작성지침

법령별 공포번호·공포일·시행일·현행/시행예정 상태와 조문 해시를 [`data/legal-source-snapshot.json`](./data/legal-source-snapshot.json)에 저장합니다. 변경이 없으면 파일이나 배포를 수정하지 않습니다. 변경이 확인되면 고정 자동화 브랜치를 갱신하고 [`reports/legal-updates/latest.md`](./reports/legal-updates/latest.md)에 공식 원문 링크, 변경된 버전·조문과 검토 체크리스트를 기록한 Draft PR을 생성합니다.
