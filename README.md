...
모든 지적에는 원문 근거, 관련 법령·조문, 수정 권고와 규칙이 근거를 직접 찾은 정도를 함께 표시합니다. 이 값은 통계적 정확도가 아닙니다.
...
- 6개 업종·24개 표본으로 오탐과 놓친 항목을 기록하는 브라우저 로컬 전용 개발자 사전 교정 작업대
...
알려진 오탐이 다시 발생하는지 확인하기 위해 법령 요소에서 만든 긍정·부정 예제로 회귀 테스트합니다. 이 예제는 엔진의 규칙 안정성을 확인하는 자료이며, 변호사·개인정보 전문가가 이중 검토한 통계적 정확도 데이터셋은 아닙니다.

### 법률 판단 정확도 평가 상태

현재는 **전문가 평가 전**입니다. URL 탐색 QA 84%와 화면의 자동탐지 기재 충족도는 법률 정확도가 아닙니다. 전문가 검토 데이터가 없어 법률 판단 성능 수치는 아직 없습니다.

평가 기준은 쉽게 말해 여섯 가지입니다. **전문가 2명이 같은 결론을 내리는지**, **회사와 업종이 치우치지 않은 충분한 사례인지**, **지적한 원문과 법적 근거가 판단을 뒷받침하는지**, **중요한 문제를 놓치지 않는지**, **문제가 아닌 내용을 과하게 경고하지 않는지**, **표본 오차를 감안해도 기준을 넘는지**를 확인합니다. 일부 발췌문, 전문가 조정이 끝나지 않은 라벨, 개발자가 만든 합성 예제는 성능 수치에서 제외합니다.

전문가가 검토한 실제 문서가 충분히 모이기 전에는 성능 기준으로 배포를 합격시키지 않습니다. 지금은 데이터 형식과 결과 재현성, 그리고 ‘아직 전문가 평가 전’이라는 정직한 표시만 자동 확인합니다. 일반 설명은 [라이브 평가 방법 페이지](https://law-lens-privacy-kr.zzboof.chatgpt.site/methodology), 개발자를 위한 상세 계약은 [`docs/legal-accuracy-evaluation.md`](./docs/legal-accuracy-evaluation.md)에서 확인할 수 있습니다.

전문가 검토 전에는 [개발자 사전 교정 작업대](https://law-lens-privacy-kr.zzboof.chatgpt.site/calibration)에서 실제 분석 결과 24건을 직접 확인합니다. 각 지적을 `맞음·오탐·판단 유보`로 나누고 분석기가 놓친 항목을 별도로 기록하지만, 이 결과는 전문가 정답 데이터나 법률 판단 정확도 수치로 사용하지 않습니다. 기록은 서버로 전송하지 않고 브라우저 IndexedDB에만 저장하며, 백업 JSON에는 처리방침 원문·발견 문구·회사명·URL을 넣지 않습니다. 데이터 계약과 쉬운 사용 설명은 [`docs/developer-calibration.md`](./docs/developer-calibration.md)에 있습니다.
...
2026년 8월 11일 주요 서비스 50개의 공개 홈페이지를 고정 코퍼스로 재검증한 결과, 예상한 공식 출처의 방침을 확인한 사례는 42개(84%)였고 예상 공식 도메인을 벗어난 출처 선택은 0개였습니다. 나머지는 `policy_not_found` 5개, 사이트 차단 2개, 상위 서버 오류 1개였습니다. 이 수치는 고정 보장값이 아니라 외부 사이트 구조와 차단 정책에 따라 달라지는 실시간 QA 결과입니다. `npm run qa:50:gate`는 공식 출처 확인 40개 이상과 도메인 수준 오탐 0개를 공개 베타 최소 기준으로 강제합니다.
...
- [x] 필드별 전문가 합의도·95% 신뢰구간·회사 단위 표본 오차 계산 구현
- [ ] 전문가 조정 문서 100개 이상 확보 후 locked-test 성능 게이트 활성화
- [ ] 이전 처리방침과 변경사항 비교
- [ ] PDF 처리방침 텍스트 추출
