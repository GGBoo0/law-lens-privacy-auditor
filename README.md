...
[![Ruleset](https://img.shields.io/badge/ruleset-KR--PRIVACY--2026.07.26-c8ff00)](./lib/legal-baseline.ts)
...
- 회사 홈페이지 또는 처리방침 URL에서 방침 링크 자동 탐색
...
- JSON 분석 보고서 다운로드
- 외부 AI API·유료 데이터 API·앱 데이터베이스 없이 요청 단위 분석
...
5. 높음·중간·낮음 발견 수를 가중해 위험 점수를 계산합니다.

기본 점수식은 `100 - 높음×12 - 중간×6 - 낮음×2`입니다. 같은 문서는 같은 규칙셋에서 최대한 비슷한 결과가 나오므로 자동화된 1차 실무 체크에 적합합니다.

## 법령 기준

규칙셋 `KR-PRIVACY-2026.07.26`은 2026년 7월 26일 국가법령정보센터와 개인정보보호위원회의 공식 원문을 기준으로 검증했습니다.
...
2026년 9월 11일 시행 예정인 개인정보 보호법 개정은 보고서에 예고만 표시하며 현재 분석 규칙에는 적용하지 않습니다. 법령 메타데이터는 [`lib/legal-baseline.ts`](./lib/legal-baseline.ts)에서 한 번에 관리합니다.
...
- 자바스크립트로만 렌더링되는 일부 처리방침은 원문 붙여넣기가 필요합니다.
...
- [ ] 표 구조 인식과 필수 열 단위 검사
- [ ] 검토자의 해당 없음·수정 완료·검토 의견 기능
- [ ] 분석 원문 해시와 규칙셋 버전 기반 감사 이력
- [ ] 이전 처리방침과 변경사항 비교
- [ ] PDF 처리방침 텍스트 추출
